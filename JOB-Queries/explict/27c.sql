...
join title AS t on (t.production_year BETWEEN 1950 AND 2010 AND t.id = mk.movie_id)
...
on(t_ml.linked_movie_id = mc.movie_id)
join
 (select movie_id from complete_cast AS cc 
join comp_cast_type AS cct1 on (cct1.kind = 'cast' AND cct1.id = cc.subject_id)
join comp_cast_type AS cct2 on (cct2.kind LIKE 'complete%' AND cct2.id = cc.status_id)) as t_cc 
on(t_cc.movie_id = t_ml.linked_movie_id)
JOIN movie_info AS mi  
on(mi.movie_id = t_cc.movie_id and mi.info IN ('Sweden', 'Norway', 'Germany', 'Denmark', 'Swedish','Denish', 'Norwegian', 'German', 'English'))