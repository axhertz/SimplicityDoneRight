...
join title AS t on (t.production_year > 2000 AND (t.title LIKE '%Freddy%' OR t.title LIKE '%Jason%' OR t.title LIKE 'Saw%') AND t.id = mk.movie_id)
join
...
on(mc.movie_id = t_mi_idx.movie_id and mc.note LIKE '%(Blu-ray)%')
join company_name AS cn on (cn.name LIKE 'Lionsgate%' AND cn.id = mc.company_id)
JOIN cast_info AS ci  
on(ci.movie_id = mc.movie_id and ci.note IN ('(writer)', '(head writer)', '(written by)', '(story)', '(story editor)'))
