...
join company_name AS cn on (cn.country_code = '[us]' AND cn.name = 'YouTube' AND cn.id = mc.company_id and mc.note LIKE '%(200%)%' AND mc.note LIKE '%(worldwide)%')
join company_type AS ct on (ct.id = mc.company_type_id)) as t_mc 
on(t_mc.movie_id = at.movie_id)
JOIN movie_keyword AS mk  
on(mk.movie_id = t_mc.movie_id)
