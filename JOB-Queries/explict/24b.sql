...
join title AS t on (t.production_year > 2010 AND t.title LIKE 'Kung Fu Panda%' AND t.id = mk.movie_id)
join
 (select movie_id from movie_companies AS mc 
join company_name AS cn on (cn.country_code ='[us]' AND cn.name = 'DreamWorks Animation' AND cn.id = mc.company_id)) as t_mc 
...
on(t_ci.movie_id = t_mc.movie_id)
JOIN aka_name AS an  
